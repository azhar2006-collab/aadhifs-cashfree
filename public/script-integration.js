...
//  Replaces the old UPI/WhatsApp manual flow
// ════════════════════════════════════════════════════════════

// ── Customer Details Modal HTML (injected dynamically) ───────
function injectCustomerModal() {
  ...
  <style>
    ...
    .cf-label {
      ...
      color: #444438; letter-spacing: 0.06em; text-transform: uppercase;
      margin-bottom: 6px;
    }
    ...
    .cf-error-msg {
      font-size: 11.5px; color: #e53e3e; margin-top: 4px; display: none;
    }
    ...
    .cf-pay-btn {
      ...
      letter-spacing: 0.03em; box-shadow: 0 4px 18px rgba(27,67,50,0.3);
      margin-top: 4px;
    }
    .cf-pay-btn:hover:not(:disabled) { background: #2D6A4F; transform: translateY(-2px); }
    .cf-pay-btn:disabled {
      background: #888; cursor: not-allowed; transform: none;
      box-shadow: none;
    }
    .cf-pay-btn .spinner {
    ...
    .cf-cancel-btn {
      ...
      border: 1px solid rgba(27,67,50,0.18); padding: 11px;
...
// ── Open checkout modal (called from cart "Order via WhatsApp" btn) ──
// We reuse the same button and intercept it
...
async function initiateCashfreePayment() {
  ...
  try {
    ...
    // Step 2: Load Cashfree JS SDK and open payment
...
// ── Detect environment from server (TEST vs PROD) ────────────
function getCashfreeMode() {
  // Reads meta tag set by server, or defaults to sandbox
  const meta = document.querySelector('meta[name="cf-env"]');
  return (meta && meta.content === 'PROD') ? 'production' : 'sandbox';
}

// ── Verify payment after SDK confirms ────────────────────────
...
function showPaymentSuccess(orderId, name, total, paymentId) {
  ...
  <style>
    ...
    .cf-success-title {
      ...
      font-weight: 600; color: #1B4332; margin-bottom: 8px;
    }
    .cf-success-msg { font-size: 14px; color: #444438; line-height: 1.6; margin-bottom: 20px; }
    .cf-success-details {
      background: rgba(27,67,50,0.06); border-radius: 10px;
      padding: 14px 16px; margin-bottom: 22px; text-align: left;
    }
    .cf-success-detail-row {
      display: flex; justify-content: space-between;
      font-size: 13px; color: #444438; padding: 4px 0;
    }
